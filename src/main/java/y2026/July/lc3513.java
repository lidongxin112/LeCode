...
public class lc3513 {
    public int uniqueXorTriplets(int[] nums) {
        ...
        // 所有可能的 XOR 值为 [0, 2^(msb(n) + 1) - 1]，
        // 因此不同值的数量为 2^(msb(n) + 1)。
        return Integer.highestOneBit(n) << 1;
    }
    public static void main(String[] args) {
        lc3513 lc = new lc3513();
        int[] nums = {3,1,2};
        int i = lc.uniqueXorTriplets(nums);
        System.out.println(i);
        System.out.println(lc.uniqueXorTriplets(new int[]{1,2}));
        System.out.println(lc.uniqueXorTriplets(new int[]{1,2,3,4}));
    }
}
