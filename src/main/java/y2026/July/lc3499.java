...
public class lc3499 {
    public int maxActiveSectionsAfterTrade(String s) {
        ...
        while (i < n) {
            ...
            // Skip one complete active block. It is tradable only when a
            // non-empty zero block exists on both sides.
            while (i < n && s.charAt(i) == '1') {
                i++;
            }
            if (leftZeros > 0 && i < n) {
                int rightEnd = i;
                while (rightEnd < n && s.charAt(rightEnd) == '0') {
                    rightEnd++;
                }
                maxGain = Math.max(maxGain, leftZeros + rightEnd - i);
            }
        }
    ...
    public static void main(String[] args) {
        lc3499 lc = new lc3499();
        System.out.println(lc.maxActiveSectionsAfterTrade("0100"));
        System.out.println(lc.maxActiveSectionsAfterTrade("01010"));
    }
}
