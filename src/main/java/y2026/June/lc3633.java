...
public class lc3633 {
    public int earliestFinishTime(int[] landStartTime, int[] landDuration, int[] waterStartTime, int[] waterDuration) {
        ...
        for (int i = 0; i < n; i++){
            ...
            for (int j = 0; j < m; j++){
                int end = Math.max(start, waterStartTime[j]) +waterDuration[j];
        ...
        for (int j = 0; j < m; j++){
            ...
            for (int i = 0; i < n; i++){
                int end = Math.max(start, landStartTime[i]) + landDuration[i];
                min = Math.min(min, end);
            }
        }
        return min;
    ...
    public static void main(String[] args) {
        ...
        int[] waterDuration = {3};
        System.out.println(new lc3633().earliestFinishTime(landStartTime, landDuration, waterStartTime, waterDuration));
    }
}
